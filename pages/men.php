...
<body>
    ...
    <div class="container mt-4">
        <h2 class="mb-4 text-center">Men's Category</h2>
        
        <!-- Fetch and Display Subcategories -->
        <?php
        $sqlSubcategories = "SELECT DISTINCT sub_category FROM products WHERE category = 'Men'";
        ...
        if ($resultSubcategories->num_rows > 0) {
            ...
            while ($row = $resultSubcategories->fetch_assoc()) {
                ...
                echo '<a href="men.php?subcategory=' . urlencode($subCategory) . '" class="btn btn-outline-primary subcategory-btn">' . $subCategory . '</a>';
            }
            echo '</div>';
        }
        ?>
        ...
        if ($selectedSubcategory) {
            $sqlSecondSubcategories = "SELECT DISTINCT second_sub_category FROM products WHERE category = 'Men' AND sub_category = ?";
            ...
            if ($resultSecondSubcategories->num_rows > 0) {
                ...
                while ($row = $resultSecondSubcategories->fetch_assoc()) {
                    ...
                    echo '<a href="men.php?subcategory=' . urlencode($selectedSubcategory) . '&second_subcategory=' . urlencode($secondSubCategory) . '" class="btn btn-outline-secondary second-subcategory-btn">' . $secondSubCategory . '</a>';
                }
                echo '</div>';
            }
            $stmt->close();
